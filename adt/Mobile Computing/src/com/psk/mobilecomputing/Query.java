...
public class Query{
	 ...
	 * for(int i=0;i<buffer.length;i=i+2){
	    if(MainActivity.availableFileChunks.get(buffer[i])!=null){
		HashMap<String,Boolean> chunkTable=MainActivity.availableFileChunks.get(buffer[i]);
	 ...
	 * 
       for(int i=0;i<buffer.length;i++){
          if(MainActivity.availableChunk.test(buffer[i])){
          	result.add(buffer[i]);
          	count++;
      		}
       } */
	...
	for(int i=0;i<buffer.length;i=i+2){
	    ...
	    chunkMap=MainActivity.availableChunkMap.get(buffer[i]);
	    if(chunkMap!=null){	
